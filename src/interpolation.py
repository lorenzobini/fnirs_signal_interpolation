...
def interpolate_bads_nirs(inst: Raw, bad_chs, method='nearest', exclude=()):
    """
    ...
    for i, bad in enumerate(picks_bad):
        ...
        if method == 'cubic' or method == 'quadratic':
            ...
            y_interp = f1d(x_new)
            inst.apply_function(lambda a1: y_interp, bad_chs[i], channel_wise=True)
        ...
        if method == 'bicubic':
            ...
            for index in indices_to_correct:
                ...
                if z_interp[index] > 1.0:  z_interp[index] = 1.0
                elif z_interp[index] < -1.0: z_interp[index] = -1.0

            # Overwriting the signal with the interpolated channels
            inst.apply_function(lambda a1: np.array(z_interp), bad_chs[i], channel_wise=True)
